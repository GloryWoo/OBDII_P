...
public class ImportTraceToDatabase {
    ...
    private ArrayList<DrivingEvent> eventList;
    private DBTraceDatabase traceDatabase=null;
    public ImportToDBDelegate importDelegate=null;

    public ImportTraceToDatabase(){
    }

    public void importTrace(List<StatisticsData> lsTrace, 
    						ArrayList<DrivingEvent> lsEvent) {    	
    	traceList = lsTrace;
        eventList = lsEvent;
...
//                traceDatabase.insertTrace(traceId, trace.getStartTime(), trace.getEndTime(), trace.getAverageSpeed(), trace.getAverageRotate(), trace.getAverageTemp(), (int) trace.getDistance(),
...
//                							trace.getAverage_mpg(), trace.getBattery_volt_1(), trace.getBattery_volt_2());
                traceDatabase.insertTrace(trace.tripId, trace.getStartTime(), trace.getEndTime(), trace.getAverageSpeed(), trace.getDistance(),
                		trace.startLat, trace.startLon, trace.endLat, trace.endLon, trace.scoreTheTrip, trace.scoreAllAver);                
...
//    					ArrayList<DrivingEvent> deList = entry2.getValue();
    					for (int i = 0; i < eventList.size(); i++) {
    						DrivingEvent de = eventList.get(i);
    						long tid = de.getTripid();
...
//    			}
        ...
        traceDatabase.close();
        if(importDelegate!=null)
        	importDelegate.importComplete(MSG_IMPORT_COMPLETE);
    }
}
