...
//    static final String PT_OBD_CONN = "point_obd_conn"; 
    ...
    static final String TRACE_TABLE_CREATE =
            "create table trace_table(trace_id integer primary key autoincrement," +
                    "trace_startTime text, trace_endTime text, trace_avgSpeed float, trace_avgRot int, trace_avgTemp float, trace_distance int, " + 
                    		"trace_location text, idle_time integer, night_time integer, speedy_time integer, peak_time integer, " +
                    			"time_60 integer, time_60_90 integer, time_90_120 integer, time_120 integer, " + 
                    				"total_fuel float, idle_fuel float, average_mpg float, battery_volt1 float, battery_volt2 float);";
    ...
    static final String EVENT_TABLE_CREATE = 
    		"create table event_table(event_id integer primary key autoincrement,trace_id INT8 REFERENCES trace_table(trace_id)," + 
    ...
    public void open()
    {
        String dbPath = Environment.getExternalStorageDirectory().getPath() + "/OBDII/" + DATABASE_NAME;
//        String dbPath = context.getFilesDir().getPath() + "/" + DATABASE_NAME;
    						...
    						float avg_mpg, float bat_v1, float bat_v2)
    {
        ...
        initialValues.put(TRACE_ROT, avgRot);
        initialValues.put(TRACE_TEMP, avgTemp);
        initialValues.put(TRACE_DIS, dis);
        initialValues.put(TRACE_LOC, trace_loc);
        initialValues.put(TRACE_IDLE, idle_time);
        initialValues.put(TRACE_NYTE, night_time);
        initialValues.put(TRACE_SPDY, speedy_time);
        initialValues.put(TRACE_PK, peak_time);
        initialValues.put(TRACE_60, time60);
        initialValues.put(TRACE_90, time90);
        initialValues.put(TRACE_120, time120);
        initialValues.put(TRACE_120P, time120plus);
        initialValues.put(TRACE_TOTAL_FUEL, total_fuel);
        initialValues.put(TRACE_IDLE_FUEL, idle_fuel);
        initialValues.put(TRACE_AVG_MPG, avg_mpg);
        initialValues.put(TRACE_BAT_VOL1, bat_v1);
        initialValues.put(TRACE_BAT_VOL2, bat_v2);        
    ...
    public void updateTrace(long traceId, Date endTime, float avgSpeed, int avgRot, float avgTemp, int dis){
        ...
        updateValues.put(TRACE_ROT, avgRot);
        updateValues.put(TRACE_TEMP, avgTemp);
...
//        db.setTransactionSuccessful();
    ...
    public void insertPoint(long traceId, Date time, double lat, double lon)
    {
        String dateStr = DateUtil.DateToDateStr(time);

        ContentValues cv = new ContentValues();
        cv.put(KEY_PT_TRACE, traceId);
        cv.put(PT_TIME, dateStr);
        cv.put(PT_LAT, lat);
        cv.put(PT_LON, lon);
//        cv.put(PT_SPEED, speed);
//        cv.put(PT_ROT, rot);
//        cv.put(PT_TEMP, temp);
//        cv.put(PT_ACC, acc);
    ...
    public void insertPoint(long traceId, Date time, double lat, double lon, float speed, float rot, float temp, float acc)
    {
        String dateStr = DateUtil.DateToDateStr(time);

        ContentValues initialValues = new ContentValues();
        initialValues.put(KEY_PT_TRACE, traceId);
        initialValues.put(PT_TIME, dateStr);
        initialValues.put(PT_LAT, lat);
        initialValues.put(PT_LON, lon);
//        initialValues.put(PT_SPEED, speed);
//        initialValues.put(PT_ROT, rot);
//        initialValues.put(PT_TEMP, temp);
//        initialValues.put(PT_ACC, acc);
        db.insert(POINT_TABLE, null, initialValues);
    }
    ...
    public Cursor getPointById(long id)
    {
        String sql = "Select * FROM " + POINT_TABLE + " point " + " JOIN " + TRACE_TABLE + " trace " + " ON " +
                "trace.trace_id = " + id + " AND " + " point.trace_id = " + id + ";";
        Cursor cursor = db.rawQuery(sql, null);
        if (cursor != null)
            cursor.moveToFirst();
        return cursor;
    ...
    private String getLatestTraceDate(){
        ...
        if (cursor.getCount() > 0){
            cursor.moveToFirst();
            int time = cursor.getColumnIndex(TraceDatabase.TRACE_END);
